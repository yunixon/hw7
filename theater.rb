...
class Role
  attr_accessor :name, :sex, :range1, :range2, :actors, :juri #type [man/woman]
  ...
  # Лучший актер на роль
  #def best_actor
  #  best = nil
  #  self.actors.each do |a|
  #    best = a
  #    a.actions.each do |acs|
  #      if acs.role_name == self.name && acs.rate > best.actions.rate
  #        best = a
  #      end
  #    end
  #  end
  #  puts "Role: #{self.name} Best actor: " + best.to_s
  #end
  ...
  def do_actions
    ...
    #puts "Role: #{self.name} Actors: #{self.actors}"
